...
module.exports = {
  create: (data, callback) => {
    pool.query(
      ...
      [data.name, data.location, data.coordinate, data.used, data.protocol, data.ip, data.remark],
  ...
  updateCctv: (id, data, callback) => {
    pool.query(
      `UPDATE cctv SET name=?, location=?, coordinate=?, used=?, protocol=?, ip=?, remark=? WHERE id=?`,
      [data.name, data.location, data.coordinate, data.used, data.protocol, data.ip, data.remark, id],
      (error, results, fields) => {
        if (error) {
          return callback(error);
        }
